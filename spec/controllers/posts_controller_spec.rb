...
describe Slate::PostsController do
  before :each do
    @blog = create(:blog)
  end
  ...
  describe "GET tag" do
    it "populates an array of tagged posts" do
      posts_count = 5
      posts = create_list(:post, posts_count, blog_id: @blog.id)
      get :tag, :use_route => :slate, :tag_name => "common"
      assigns(:posts).should eq posts.reverse
    end

    it "renders the :tag view" do
      get :tag, :use_route => :slate
  ...
  describe "PUT update" do
    ...
    context "with valid attributes" do
      it "changes blog's attributes" do
        put :update, id: @post.id, :use_route => :slate,
      ...
      it "redirects to the site root" do
        put :update, :use_route => :slate, id: @post.id, post: attributes_for(:post)
        response.should redirect_to root_url
      end
    end
    context "with invalid attributes" do
  ...
  describe "DELETE destroy" do
    ...
    it "deletes the post" do
      expect{
        delete :destroy, :use_route => :slate, id: @post.id
      }.to change(Slate::Post, :count).by(-1)
    end

    it "redirects to the site root" do
      delete :destroy, :use_route => :slate, id: @post
      response.should redirect_to root_url
    end
  end
end