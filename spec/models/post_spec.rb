...
describe Slate::Post do
  ...
  it "should be possible to publish more than one post" do
    ...
    expect(Slate::Post.all.count).to eq(2)
  end
  ...
  describe ".recent" do
    ...
    it "should not include drafts" do
      ...
      blog_id = Slate::Post.first.blog_id
      create_list(:post_draft, drafts_count, blog_id: blog_id)
      expect(Slate::Post.recent({}).count).to eq posts_count
    end
  end
  ...
  describe ".drafts" do
    it "should return only drafts" do
      ...
      blog_id = Slate::Post.first.blog_id
      create_list(:post_draft, drafts_count, blog_id: blog_id)
      expect(Slate::Post.drafts({}).count).to eq drafts_count
    end
  end
  ...
  describe ".tagged" do
    it "should return tagged posts" do
      ...
      expect(Slate::Post.tagged({ :tag_name => "common" }).count).to eq posts_count
    end
